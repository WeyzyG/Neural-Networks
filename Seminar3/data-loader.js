// data-loader.js
// CSV parsing, normalization, batching utilities, drawing helpers.
// Adds noise generators and helpers for autoencoder training.
...
export function splitTrainVal(xs, ys, valRatio = 0.1) {
  const n = xs.shape[0];
  const val = Math.floor(n * valRatio);
  const train = n - val;
  const trainXs = xs.slice([0, 0, 0, 0], [train, 28, 28, 1]);
  const trainYs = ys.slice([0, 0], [train, 10]);
  const valXs = xs.slice([train, 0, 0, 0], [val, 28, 28, 1]);
  const valYs = ys.slice([train, 0], [val, 10]);
...
export function addSaltPepper01(xs01, p = 0.1) {
  return tf.tidy(() => {
    const rnd = tf.randomUniform(xs01.shape);
    const zerosMask = rnd.less(p / 2).toFloat();
    const onesMask = rnd.greater(1 - p / 2).toFloat();
    const keepMask = tf.onesLike(zerosMask).sub(zerosMask).sub(onesMask).clipByValue(0, 1);
    return xs01.mul(keepMask).add(onesMask).clipByValue(0, 1);
  });
}
